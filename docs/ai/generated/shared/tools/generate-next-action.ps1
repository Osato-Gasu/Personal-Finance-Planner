...
$rulesVersion=Value ([IO.File]::ReadAllText($lockPath)) 'version' 'docs/ai/SHARED_RULES.lock.yml'
$rulesCommit=Value ([IO.File]::ReadAllText($lockPath)) 'commit' 'docs/ai/SHARED_RULES.lock.yml'
...
if($active.Count -eq 1){
    ...
    $zeroModel=if($adapter.ZeroActive -and $adapter.ZeroActive.Model){[string]$adapter.ZeroActive.Model}else{'none'}
    $zeroEffort=if($adapter.ZeroActive -and $adapter.ZeroActive.Effort){[string]$adapter.ZeroActive.Effort}else{'none'}
    $values=[ordered]@{
...
if($values.executor_policy -notin @('preferred_fallback','strict')){throw "Invalid executor_policy: $($values.executor_policy)"}
