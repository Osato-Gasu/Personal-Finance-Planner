...
Assert ($version -ceq '0.12.25' -and $manifest -match '(?m)^version:\s*0\.12\.25\s*$') 'finding-ID compatibility version checkpoint'
...
Assert ($relayTool-match'function Test-FindingId'-and$relayTool-match'Read-OpenImplementationFindingIds'-and$relayTool-match'Read-HandoffFindingIds'-and$relayTool-match'not\(Test-FindingId \(\[string\]\$finding\.id\)\)') 'bundle, open registry, and handoff finding IDs share one canonical grammar'
...
try{
    ...
    foreach($validFindingId in @('FINDING-009-01','FINDING-009-R1-01','FINDING-009-R2-01','FINDING-009-R3-01','FINDING-159-V3-01')){
        $validBundle=(($bundle|ConvertTo-Json -Depth 30)|ConvertFrom-Json);@($validBundle.findings)[0].id=$validFindingId;$validBundlePath=Join-Path $taskHistoryRoot "valid-$validFindingId.json";[IO.File]::WriteAllText($validBundlePath,($validBundle|ConvertTo-Json -Depth 30),$utf8NoBom);$validSha=(Get-FileHash -Algorithm SHA256 -LiteralPath $validBundlePath).Hash;$validBytes=(Get-Item -LiteralPath $validBundlePath).Length;$validResult=Invoke-ScriptResult (Join-Path $seed 'docs/ai/generated/shared/tools/relay-bundle.ps1') @('-Action','Validate','-ProjectRoot',$seed,'-BundlePath',$validBundlePath,'-ExpectedSha256',$validSha,'-ExpectedBytes',[string]$validBytes)
        Assert ($validResult.ExitCode-eq0) "canonical finding ID validates: $validFindingId"
    }
    $preflightSources=@(
    ...
    function Assert-FindingParserContextRejected([string]$Name,[string]$OpenValue,[string[]]$RequiredChanges){
        $project=Join-Path $taskHistoryRoot "finding-parser-$Name";$null=@(& git clone --no-checkout --quiet $convergenceProject $project 2>&1);if($LASTEXITCODE-ne0){throw "finding parser fixture clone failed: $Name"};$null=Invoke-Git $project @('config','core.autocrlf','false') "finding parser autocrlf $Name";$null=Invoke-Git $project @('config','user.name','finding parser simulation') "finding parser user $Name";$null=Invoke-Git $project @('config','user.email','finding-parser@example.invalid') "finding parser email $Name";$null=Invoke-Git $project @('checkout','-q','codex/task-004') "finding parser checkout $Name";$null=Invoke-Git $project @('remote','set-url','origin','https://github.com/owner/fixture.git') "finding parser remote $Name"
        if($null-ne$OpenValue){$taskPath=Join-Path $project 'docs/ai/tasks/TASK-004.md';$taskText=[IO.File]::ReadAllText($taskPath);$taskText=[regex]::Replace($taskText,'(?m)^implementation_review_open_finding_ids:.*$',"implementation_review_open_finding_ids: $OpenValue");[IO.File]::WriteAllText($taskPath,$taskText,$utf8NoBom)}
        if($null-ne$RequiredChanges){Set-ConvergenceContext $project $RequiredChanges @('none')}
        Commit-Convergence $project "finding parser $Name";$caseHead=(Invoke-Git $project @('rev-parse','HEAD') "finding parser head $Name")[-1].Trim();$caseFinding=[pscustomobject]@{id='FINDING-154-R2-01';severity='MAJOR';target='validator';problem='parser case';evidence='parser evidence';impact='parser impact';required_change='parser change';review_scope='accepted_prior_finding';prior_finding_id='FINDING-154-01'};$caseBundle=Join-Path $taskHistoryRoot "finding-parser-$Name.json";Write-ConvergenceBundle $caseBundle $bundle $caseFinding 1 $caseHead $null;Invoke-ConvergenceImport $project $caseBundle "finding parser rejects $Name" 0 0 'none' 'none' $false|Out-Null
    }
    Assert-FindingParserContextRejected 'duplicate-open' 'FINDING-154-R1-01, FINDING-154-R1-01' $null
    Assert-FindingParserContextRejected 'malformed-open' 'FINDING-154-R1-X' $null
    Assert-FindingParserContextRejected 'malformed-required' $null @('FINDING-154-R1-X [MAJOR] malformed finding')
    Assert-FindingParserContextRejected 'duplicate-required' $null @('FINDING-154-01 [MAJOR] first finding','FINDING-154-01 [MAJOR] duplicated finding')
    ...
    $f2=[pscustomobject]@{id='FINDING-154-R2-01';severity='MAJOR';target='validator';problem='unresolved first finding';evidence='second evidence';impact='second impact';required_change='second change';review_scope='accepted_prior_finding';prior_finding_id='FINDING-154-01'};Write-ConvergenceBundle $convergenceBundlePath $bundle $f2 1 $head $null;Invoke-ConvergenceImport $convergenceProject $convergenceBundlePath 'attempt 2 failure alone enables attempt 3 terminal with canonical review-cycle finding ID' 2 3 'terminal' 'false' $true|Out-Null;Commit-Convergence $convergenceProject 'convergence attempt two'
    ...
    $f3=[pscustomobject]@{id='FINDING-154-R3-01';severity='MAJOR';target='release gate';problem='release blocker';evidence='third evidence';impact='release blocked';required_change='blocker change';review_scope='release_gate';prior_finding_id=$null};Write-ConvergenceBundle $convergenceBundlePath $bundle $f3 1 $head $null
    $terminalBefore=Get-ProjectByteSnapshot $convergenceProject;$terminalSha=(Get-FileHash -Algorithm SHA256 -LiteralPath $convergenceBundlePath).Hash;$terminalBytes=(Get-Item -LiteralPath $convergenceBundlePath).Length;$terminalArgs=@('-ProjectRoot',$convergenceProject,'-BundlePath',$convergenceBundlePath,'-ExpectedSha256',$terminalSha,'-ExpectedBytes',[string]$terminalBytes);$terminalValidate=Invoke-ScriptResult (Join-Path $convergenceProject 'docs/ai/generated/shared/tools/relay-bundle.ps1') (@('-Action','Validate')+$terminalArgs);$terminalAfterValidate=Get-ProjectByteSnapshot $convergenceProject;$terminalValidateStatus=@(& git -C $convergenceProject status --porcelain=v1 --untracked-files=all)
    Assert ($terminalValidate.ExitCode-eq0-and(Test-ProjectByteSnapshot $terminalBefore $terminalAfterValidate)-and$terminalValidateStatus.Count-eq0) 'terminal review-cycle bundle Validate is read-only and succeeds'
    $terminalRollback=Invoke-ScriptResult (Join-Path $convergenceProject 'docs/ai/generated/shared/tools/relay-bundle.ps1') (@('-Action','Import')+$terminalArgs+@('-FailureInjection','after_next_action'));$terminalAfterRollback=Get-ProjectByteSnapshot $convergenceProject;$terminalRollbackStatus=@(& git -C $convergenceProject status --porcelain=v1 --untracked-files=all)
    Assert ($terminalRollback.ExitCode-ne0-and$terminalRollback.Output-match'injected relay failure'-and(Test-ProjectByteSnapshot $terminalBefore $terminalAfterRollback)-and$terminalRollbackStatus.Count-eq0) 'terminal review-cycle Import failure rolls back bytes, directories, and git status exactly'
    Invoke-ConvergenceImport $convergenceProject $convergenceBundlePath 'attempt 3 review-cycle finding terminates in NEEDS_USER_DECISION' 3 3 'terminal' 'true' $true|Out-Null
    $terminalTask=[IO.File]::ReadAllText((Join-Path $convergenceProject 'docs/ai/tasks/TASK-004.md'));$terminalState=[IO.File]::ReadAllText((Join-Path $convergenceProject 'docs/ai/CURRENT_STATE.md'));$terminalNext=[IO.File]::ReadAllText((Join-Path $convergenceProject 'docs/ai/NEXT_ACTION.yml'));$terminalHandoffRelative=Read-ConvergenceValue $terminalTask 'handoff_file' 'terminal TASK';$terminalHandoff=[IO.File]::ReadAllText((Join-Path $convergenceProject $terminalHandoffRelative));$terminalCanonical=[IO.File]::ReadAllText((Join-Path $convergenceProject 'docs/ai/reports/TASK-004/RELAY_BUNDLE.json'))
    $terminalIdentityExact=(Read-ConvergenceValue $terminalTask 'status' 'terminal TASK')-ceq'needs_user_decision'-and(Read-ConvergenceValue $terminalTask 'current_phase' 'terminal TASK')-ceq'user_decision'-and(Read-ConvergenceValue $terminalNext 'phase' 'terminal NEXT_ACTION')-ceq'user_decision'-and(Read-ConvergenceValue $terminalNext 'next_actor' 'terminal NEXT_ACTION')-ceq'ChatGPT'-and(Read-ConvergenceValue $terminalNext 'next_role' 'terminal NEXT_ACTION')-ceq'ORCHESTRATOR_AND_REVIEWER'-and(Read-ConvergenceValue $terminalNext 'user_confirmation_required' 'terminal NEXT_ACTION')-ceq'true'-and(Split-Path -Leaf $terminalHandoffRelative)-ceq'USER_DECISION_HANDOFF.md'
    $terminalFindingExact=$terminalHandoff-match'(?m)^- source_decision: CHANGES_REQUESTED$'-and$terminalHandoff-match'(?m)^- FINDING-154-R3-01 \[MAJOR\] release gate: release blocker Evidence: third evidence Impact: release blocked Required: blocker change$'-and$terminalCanonical-match'"id":\s*"FINDING-154-R3-01"'-and$terminalTask-match'(?m)^implementation_review_open_finding_ids: FINDING-154-R3-01$'
    Assert ($terminalIdentityExact-and$terminalFindingExact-and($terminalTask+$terminalState+$terminalNext+$terminalHandoff)-notmatch'(?i)attempt\s*4') 'terminal Import materializes cycles 3, attempt 3, NEEDS_USER_DECISION, full R3 finding, user confirmation, and no attempt 4'
    Commit-Convergence $convergenceProject 'convergence terminal decision'
