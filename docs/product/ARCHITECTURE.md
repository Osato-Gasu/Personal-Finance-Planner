...
current schemaはv10、current storage keyは`personal-finance-planner:state:v10`とする。v10が存在する場合はparse失敗をfail closedとし、旧keyへfallbackしない。v10がなくv9以下が存在する場合だけ決定的なmigrationを実行し、legacy bytesを保持したままv10をatomic saveする。v9→v10では各`PayrollPlan`へ`legacy-monthly` modeと日額800円だけを追加し、旧月額、`commutingFuelEstimate`、賞与metadata、binding、他domainを変更しない。
...
TASK-018の通勤手当authorityはdomainのsingle selectorに限定する。`legacy-monthly`は旧月額、`car-daily`は平均出勤日数×日額のBigInt half-up、`none`は0円を返し、`calculatePayroll()`と給与→手取りは必ずその結果だけを使う。ガソリン見積はpractical-income表示に隔離し、`none`はfuel completenessより先に通勤手当・ガソリン・通勤収支を0へshort-circuitする。UIはlegacy checkboxをmixedとして表示し、明示操作までmodeを保持する。
