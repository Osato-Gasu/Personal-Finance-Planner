...
effectiveMonthlyNonTaxableCommutingYen =
  legacy-monthly: persisted monthlyNonTaxableCommutingYen
  car-daily: round-half-up(averageWorkdaysPerMonthTenths * dailyYen / 10)
  none: 0
monthlyGrossYen = monthlyTaxableSalaryYen + effectiveMonthlyNonTaxableCommutingYen
annualTaxableSalaryYen = monthlyTaxableSalaryYen * 12 + sum(bonuses)
annualGrossYen = annualTaxableSalaryYen + effectiveMonthlyNonTaxableCommutingYen * 12
```

`car-daily`の乗算・half-upはBigIntだけで行い、0.5円境界を上へ丸める。`monthlyNonTaxableCommutingYen`は`legacy-monthly`以外では保存互換bytesに限り、derived値を書き戻さない。入力日額は現行モデルで非課税通勤手当として扱う値であり、法定の距離別非課税限度額・課税超過分はTASK-018で自動判定しない。
...
commutingBalanceYen = effectiveMonthlyNonTaxableCommutingYen - estimatedGasolineYen
...
途中丸めを行わず、最終月額だけを1円へhalf-upし、同じ丸め済み値を通勤収支・実質月収・年収へ再利用する。`car-daily`の燃料未入力はガソリン関連のpractical resultだけを未計算とし、法定給与と給与→手取りは日額通勤手当から計算を継続する。`none`は燃料入力の完全性に関係なく、通勤手当・ガソリン・通勤収支をexact 0、実質月収を月収と同額にする。燃料入力と試算結果は`calculatePayroll()`、給与→手取りcompensation、税、社会保険、雇用保険、bindingを変更しない。
