[CmdletBinding()]
param(
    [ValidateSet('RepositoryState','ExternalRelayPreflight','CanonicalRelay')]
...
function Assert-Classification($Provenance) {
    ...
    Assert-True ($record.Sha256 -ceq [string]$Provenance.managed_adoption.phase_a_classification_sha256) 'classification artifact SHA changed'
    ...
    Assert-True ([string]$classification.baseline_commit -ceq [string]$Provenance.phase_a_baseline.commit -and [string]$classification.baseline_tree -ceq [string]$Provenance.phase_a_baseline.tree) 'classification baseline is invalid'
...
$provenanceRecord = Read-CanonicalFile 'docs/ai/reports/TASK-013/RECOVERY_PROVENANCE.json'
...
if ([string]$provenance.phase_state -ceq 'PHASE_A_PRE_SYNC') {
    Assert-True ([string]$provenance.recovery_candidate.commit -ceq 'none' -and [string]$provenance.recovery_candidate.tree -ceq 'none') 'Phase A must not materialize recovery candidate B'
    Assert-True ($task.Text -match '(?m)^candidate_commit: 1285f6745062545bb4e73a937cde141f6ab620d4$' -and $task.Text -match '(?m)^candidate_tree: 3fb36efc2fd13b9321baf11a63e798d54fe48a12$') 'Phase A TASK candidate is not the immutable product candidate'
    $workingAgents = [IO.File]::ReadAllBytes((Resolve-ProjectPath 'AGENTS.md'))
    Assert-True ((Get-Sha256 $workingAgents) -ceq '0E146BAC3986603F586F443536E8AA4E220C63FF1C4BAE14DFB2D13C1E4282F0') 'Phase A changed managed AGENTS target before semantic approval'
} else {
    $recovery = $provenance.recovery_candidate
    Assert-True ([string]$recovery.commit -match '^[0-9a-f]{40}$' -and [string]$recovery.tree -match '^[0-9a-f]{40}$') 'recovery candidate identity is incomplete'
    Assert-True ((Get-GitTree ([string]$recovery.commit)) -ceq [string]$recovery.tree) 'recovery candidate tree changed'
    ...
    Assert-True ($task.Text -match "(?m)^candidate_commit: $([regex]::Escape([string]$recovery.commit))$" -and $task.Text -match "(?m)^candidate_tree: $([regex]::Escape([string]$recovery.tree))$") 'current TASK candidate does not equal recovery B'
    $lock = Read-CanonicalFile 'docs/ai/SHARED_RULES.lock.yml'
    Assert-True ($lock.Text -match '(?m)^source_version: 1\.0\.1$' -and $lock.Text -match "(?m)^source_commit: $([regex]::Escape([string]$shared.commit))$" -and $lock.Text -match "(?m)^source_tree: $([regex]::Escape([string]$shared.tree))$") 'shared v1.0.1 lock identity is invalid'
}

if ($Mode -ne 'RepositoryState') {
